...
$(document.body).on("change",".service_class",function(){
    ...
    if ($(this).val().length == 0) {
        $("#create_appointment_form input[name='price']").val(0);
    }
    else {
        $.ajax({
            headers: {
            ...
            success: function(result){
                $(".invalid-div span").html('');
                $("#create_appointment_form input[name='price']").val(result.data.price);
            },
            error: function(err){}
        });
    }
...
function create_booking(formID,url) {
    document.getElementById("create_btn").disabled = true;
    var formData = new FormData($('#'+formID)[0]);
    $.ajax({
        headers: {
...
function edit_service(id,base_url) {
    $.ajax({
        ...
        success: function(result){
            ...
            $("#edit_service_sidebar textarea[name='desc']").html(result.data.service.description);
...
function show_booking(id,base_url,page) {
    $.ajax({
        ...
        success: function(result){
            document.getElementById('user_email').innerHTML = result.data.booking.email;
            document.getElementById('user_name').innerHTML = result.data.booking.name;
            document.getElementById('user_phone').innerHTML = result.data.booking.phone;
            // $('#show_booking_sidebar .user_img').attr('src', base_url+'/storage/images/users/'+result.data.booking.user.image);

            document.getElementById('app_payment').innerHTML = result.data.symbol+''+result.data.booking.price;
