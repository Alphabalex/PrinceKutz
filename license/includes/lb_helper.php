...
class LicenseBoxAPI{
	...
	public function __construct(){ 
		$this->product_id = '14576588';
		$this->api_url = 'https://licencefly.com/';
		$this->api_key = '9F2E8E3505307C-singlbrbr';
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["03fcdf5fd2d83e2"])){
				$_SESSION["03fcdf5fd2d83e2"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["03fcdf5fd2d83e2"])){
				$get_data = $this->call_api(
					'POST',
